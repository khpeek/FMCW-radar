...
''' Time grid '''
t=(-N/2.+n)*Ts              
NFFT=nextpow2(N+fs**2/alpha)       # Number of FFT points required to avoid time-domain aliasing in the output of deskew filter
...
_,SIF_lin=Fourier(wIF*sIF_lin,fs,NFFT)              # Ideal beat spectrum 
...
#scale=1.5
#plt.xlim(np.array([fb-scale*fsl,fb+scale*fsl])/1.e6)
